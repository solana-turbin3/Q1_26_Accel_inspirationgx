...
describe("whitelist-transfer-hook", () => {
  ...
  it("Initializes the Whitelist", async () => {
    const tx = await program.methods
      .initializeWhitelist(provider.publicKey)
      .accountsPartial({
  ...
  xit("Initialize task queue authority", async () => {
  ...
  it("Schedule Remove user from whitelist", async () => {
    const taskQueue = new anchor.web3.PublicKey(
      "Xzbp6k8RML93HmQApetrUTanzJMDyvaqX2ChRGrJPK8",
    );
    ...
    let taskID = 7;
    const tx = await program.methods
      .scheduleRemoveFromWhitelist(taskID)
      .accounts({
        task: taskKey(taskQueue, taskID)[0],
        taskQueue: taskQueue,
        taskQueueAuthority: taskQueueAuthority,
    ...
    console.log(
      "\n scheduled User removed from whitelist:",
      provider.publicKey.toBase58(),
    );
    console.log("Transaction signature:", tx);
  });

  xit("Remove user from whitelist", async () => {
    const tx = await program.methods
  ...
  xit("Close user whitelist", async () => {
    const tx = await program.methods
